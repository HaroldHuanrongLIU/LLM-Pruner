"""Sweep all ratios across 10 scenarios — PPL + GPU memory + latency.

Run in LLM-Pruner venv:
    cd baselines/LLM-Pruner
    .venv/bin/python rap_baseline_scripts/sweep_scenarios.py
"""
import gc
import json
import math
...
# Add LLM-Pruner root to path for unpickling saved models
...
RATIOS = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7]
SCENARIOS = [
...
def measure_mem(ratio, bs, seq_len):
...
def main():
    ...
    out_path = Path("../../outputs/dynamic_memory_comparison_v2/llmpruner.json")
    out_path.parent.mkdir(parents=True, exist_ok=True)

    results = []
    for ratio in RATIOS:
        ckpt = f"prune_log/llama2_ratio_{ratio}/pytorch_model.bin"
        if not Path(ckpt).exists():
            print(f"  ratio={ratio}: MISSING", flush=True)
            continue
        ...
        mem_at = {}
        for s in SCENARIOS:
            m = measure_mem(ratio, s["bs"], s["seq_len"])
            mem_at[s["id"]] = round(m, 2) if isinstance(m, (int, float)) else m
            print(f"    S{s['id']} (bs={s['bs']},sl={s['seq_len']}): {mem_at[s['id']]}GB", flush=True)

        results.append({"ratio": ratio, "ppl": ppl_results, "latency": latency_results, "memory_at_scenarios": mem_at})
        del model, tokenizer
